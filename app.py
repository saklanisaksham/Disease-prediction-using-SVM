...
svm_classifier = pickle.load(open('svm_classifier_model.pkl', 'rb'))

with st.sidebar:
    selected = option_menu('Disease Prediction System',

                           ['Disease Prediction system using SVM'],
...
def predict_note_authentication(symptoms):
    # Fill the remaining symptoms with null values (0 in this case)
    symptoms += [0] * (17 - len(symptoms))
    symptoms = list(map(int, symptoms))
    predictions = svm_classifier.predict([symptoms])
    print(predictions)
    return predictions
...
def main():
    ...
    <h2 style="color:white;text-align:center;">Mini Project : Disease Prediction ML App </h2>
    ...
    # Create a search bar for symptoms
    selected_symptoms = st.multiselect("Select Symptoms:", list(symptom_weights.keys()))

    # Convert selected symptoms to their weights
    symptoms_weights = [symptom_weights[symptom] for symptom in selected_symptoms]

    result = ""
    if st.button("Predict"):
        result = predict_note_authentication(symptoms_weights)

    st.success(f'The output is {", ".join(result)}')
